...
- By default, labels are loaded from `fastapi_service/labels.json`. Its order must match `train_ds_raw.class_names` from the training notebook.
- If no labels are provided, output names are returned as `class_<index>`.
- `/predict` uses multi-crop + horizontal-flip averaging at inference time. This makes predictions less sensitive to phone-image framing than a single center crop, but real-world reliability still requires training/validation images collected outside the clean PlantVillage-style dataset.
